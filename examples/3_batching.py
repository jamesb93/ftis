...
parser.add_argument(
    "-o",
    "--output",
    default="~/corpus-folder/batching",
    type=str,
...
process = FTISProcess(source=src, sink=out, mode="batch")  # set the mode to batch

# The process will now run the Corpus input to each of the analysers in parallel
process.add(FluidNoveltyslice(), FluidMFCC(), FluidLoudness())
